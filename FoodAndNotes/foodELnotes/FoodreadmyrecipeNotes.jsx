...
const FoodreadmyrecipeNotes = ({ recipe }) => {
    ...
    const deleteMyRecipe = async () => {
        ...
        try {
            const savedRecipes = await AsyncStorage.getItem('MY_RECIPES_DUTCH');
        ...
        return (
            ...
            >
                <View style={[cmnstyle.row, { marginBottom: 8, alignItems: 'flex-start' }]}>
                    {isStep ? (
                        <>
                            <Text style={[readfood.ingredientTitle, {marginRight: 12}]}>{item.step} step</Text>
                            <Text style={[readfood.ingredientText, {width: '80%'}]}>{item.description}</Text>
                        </>
                    ) : (
                        <>
                            <Text style={[readfood.ingredientTitle, {marginRight: 12}]}>{item.title}</Text>
                            <Text style={[readfood.ingredientText, {width: '80%'}]}>{item.quantity}</Text>
                        </>
                    )}
                </View>
            </Animated.View>
        ...
        <Animated.View style={{ width: '100%', height: '100%', opacity: fadeAnim }}>
            ...
            >
                <View style={cmnstyle.row}>
                    <TouchableOpacity
                        onPress={() => navigation.navigate('FoodmyrecipesNotesEL')}
                ...
                <TouchableOpacity
                    ...
                    onPress={() => navigation.navigate('FoodcreaterecipeNotesEL', { recipe })}
                    hitSlop={{ top: 20, bottom: 20, left: 20, right: 20 }}
                >
            ...
            >
                ...
                >
                    <Animated.View style={{ paddingHorizontal: 20, opacity: fadeAnim }}>
                        <Text style={readfood.region}>{recipe.kitchen_type}</Text>
                        <Text style={readfood.title}>{recipe.title}</Text>
                        <View style={cmnstyle.row}>
                            <Text style={foodcard.types}>{recipe.difficulty_level}</Text>
...
export default FoodreadmyrecipeNotes;